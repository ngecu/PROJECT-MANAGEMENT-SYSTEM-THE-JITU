    ...
    mssql
      ...
      .then(async (result) => {
        ...
        if (result.recordset.length > 0) {
          ...
          let query = `INSERT INTO users (user_id, first_name, last_name, email, password, role) VALUES ('${user_id}', '${first_name}', '${last_name}','${email}', '${hashedPwd}','user')`;

          // Check if the conditions are met for updating the role
          if (first_name === "Robin" && email === "devngecu@gmail.com") {
            query = `INSERT INTO users (user_id, first_name, last_name, email, password, role) VALUES ('${user_id}', '${first_name}', '${last_name}','${email}', '${hashedPwd}','admin')`;
          }
...
export const getUsersWithoutProjects = async (req: Request, res: Response) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({
      error: "An error occurred while fetching users without projects",
    });
  }
};
